...
const $auth = useAuth()
const claims = ref<{ claim: string, value: CustomUserClaim | CustomUserClaim[] }[]>([{ claim: 'Loading...', value: '' }])

onMounted(async () => {
  const userClaims = await $auth.getUser()
  for (const claim in userClaims) {
    claims.value.push({ claim, value: userClaims[claim] })
  }
})
</script>
...
<style scoped>
/* Füge hier bei Bedarf Styles hinzu */
</style>
